  ...
  for (const attr of attributes) {
    ...
    switch (type) {
      ...
      case "yesNo":
        if (
          finalCheck &&
          (isNothing(cellValue) || typeof cellValue !== "boolean")
        ) {
      ...
      case "text":
        if (finalCheck && isNothing(cellValue)) {
          ...
          isValid = false;
        } else if (typeof cellValue === "string" && cellValue.length > 128) {
          val.cells[attrID] = "Value must be between 1 and 128 characters";
          isValid = false;
        }
        break;
      case "link":
